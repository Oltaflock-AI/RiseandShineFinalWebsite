      ...
      <Container>
        ...
        {toA && fromA && (
          <p className="mt-2 text-white/80">
            {fromA.city} ({fromA.code}) → {toA.city} ({toA.code}) · {adults} adult{adults > 1 ? "s" : ""} · {trip === "round" ? "Round-trip" : "One-way"}
          </p>
        )}
      </Container>
      <div className="mt-6">
        ...
        <Container>
          {!res.ok ? (
            <div className="mx-auto max-w-lg rounded-brand-lg border border-line bg-white p-8 text-center shadow-brand-sm">
              ...
              <h2 className="h-sm mb-2">Live fares are unavailable right now</h2>
              <p className="mb-6 text-muted">
                We couldn&apos;t reach the flight system for {fromA?.city} → {toA.city}
                {" "}just now. Send us the trip and our team will get you the best fare by hand.
              </p>
